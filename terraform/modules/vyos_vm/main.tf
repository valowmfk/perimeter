...
###############################################
# VyOS Cloud-Init VM (BPG provider)
###############################################
...
resource "proxmox_virtual_environment_vm" "vyos" {
  ...
  ###############################################
  # Cloud-init
  # VyOS doesn't process network-config v1 properly (ignores static IP,
  # falls back to DHCP). Only use cloud-init for user/SSH key injection.
  # Network config is handled by SSH bootstrap post-clone.
  ###############################################
  initialization {
    datastore_id = var.datastore_id
