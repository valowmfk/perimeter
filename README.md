...
4. Restart the `perimeter` and `perimeter-worker` services
5. Run a health check against the `/api/version` endpoint

If upgrading from v3.1, the worker service file may need to be regenerated. The script will prompt you if this is needed.
...
### v3.2 (2026-03-25)
- **Code consolidation** — Deduplicated ~450 lines across Python and JavaScript (shared utilities for SSH wait, VM types, tfvars I/O, bridges parsing, cascade loaders, flyout toggles, IP validation)
- **Hardcoded values removed** — Playbooks use Ansible variables, workflows require subnet config, JS reads defaults from backend
- **Version endpoint** — `GET /api/version` returns version, node, homelab name, and enabled features
- **Homelab identity** — Name your lab in the installer; appears in the web UI header and VM MOTD banners
- **Upgrade script** — `installer/upgrade.sh` for clean version-to-version upgrades
- **Worker service** — Celery worker service now generated by the installer alongside the main service
- **Bug fixes** — Duplicate focus trap on cert modal, Proxmox API null safety, PM headers reconstruction

### v3.1 (2026-03-25)
- Celery/Redis job queue replacing subprocess workers
- Fleet deployments (Quick Fleet + Custom Fleet flyout)
- Full `qbranch` → `perimeter` rename
- A10 theme removed (single Perimeter theme)
- Installer hardening (Ubuntu 25.10, venv, PEP 668, Age key management)
- CSP-compatible (no inline scripts)

### v3.0 (2026-03-24)
- Initial public release with install script and setup wizard
